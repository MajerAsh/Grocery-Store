...
/**
 * //1Prints out the name of each item in the given array w forEach
...
function getUppercaseNames(items) {
  const mappeduppercase = items.map((foodObj) => foodObj.name.toUpperCase());
...
function getItemsByCategory(items, category) {
  const filteredcat = items.filter((item) => item.category === category);
  return filteredcat;
}
